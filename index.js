...
const  verifyjwt = (req,res,next)=>{
  ...
  if(!authorization){
    return res.status(401).send({error: true , message: "unauthorized eccess"})
  }

  // beareer token 

  const token = authorization.split(" ")[1];
  jwt.verify(token,process.env.ACCCSS_TOKEN_SECRET,(err,decoded)=>{
    if(err){
...
async function run() {
 ...
 // jwt  setup 
   ...
   app.post("/jwt", (req,res)=>{
    const usre = req.body;
    const token = jwt.sign(usre,process.env.ACCCSS_TOKEN_SECRET, { expiresIn: '1h' })
    res.send({token})
   })
